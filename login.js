...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (adminDoc.exists) {
            ...
            loginMessage.textContent = 'Admin login successful! Redirecting to dashboard...';
            loginMessage.classList.remove('hidden');
...
document.getElementById('signupForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        signupMessage.textContent = 'Account created successfully! You are now logged in. Redirecting to home...';
        signupMessage.classList.remove('hidden');
        
        // Close modal and redirect - user is already logged in
        setTimeout(() => {
            document.getElementById('signupModal').classList.add('hidden');
            window.location.href = 'index.html';
        }, 1500);
...
window.addEventListener('load', () => {
    ...
    if (auth) {
        // IMPORTANT: Removed auto-redirect on page load
        // Users can now access the login page even if logged in
        // The redirect only happens after clicking "Sign In" button
